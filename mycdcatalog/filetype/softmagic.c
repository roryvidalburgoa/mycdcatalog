	...
	int nbytes;
{
	...
	for (magindex = 0; magindex < nmagic; magindex++) {
		...
		if (magic[magindex].desc && (strlen(magic[magindex].desc) > 0)) {
		    tmpoff[cont_level] =
			mprint(&p, &magic[magindex]) , levels++;
		}
		...
		//if (magic[magindex+1].cont_level != 0 && 
		       ++magindex < nmagic) {//while continuations
			...
			if (cont_level >= magic[magindex].cont_level) {
				...
				if (mget(&p, s, &magic[magindex], nbytes) &&
				    mcheck(&p, &magic[magindex])) {//mget & mcheck
					/*
