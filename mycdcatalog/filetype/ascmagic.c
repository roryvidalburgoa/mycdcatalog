	...
	int nbytes;	/* size actually read */
{
	...
	case 1:
		sprintf(acode, "%s", "TAR");
		sprintf(adesc, "%s", "tar archive");
		return 1;
	case 2:
		sprintf(acode, "%s", "PTAR");
		sprintf(adesc, "%s", "POSIX tar archive");
		return 1;
	}
...
subtype_identified:
	...
	for (i = 0; i < ulen; i++) {
		...
		if (subtype) {
		    ...
		    printf("Desc: %s\t",subtype);*/
			sprintf(adesc, "%s", subtype);
		}

		/*ckfputs(" ", stdout);
		ckfputs(type, stdout);*/
		if (code && type) {
			sprintf(acode,"%s %s", code, type);
			if (!subtype) {
				 sprintf(adesc, "%s %s", code, type);
			}
		}
